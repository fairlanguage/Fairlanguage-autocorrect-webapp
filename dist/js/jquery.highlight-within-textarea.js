...
(function($) {
	...
	HighlightWithinTextarea.prototype = {
		...
		generate: function() {
			...
			this.$front = $('<div>', { class: ID + '-front' })
				.append(this.$highlights.clone());

			this.$front.click(function() {
				el.elementFromPoint(20, 10).click();
			})

			this.$container = $('<div>', { class: ID + '-container' })
				.insertAfter(this.$el)
				.append(this.$backdrop, this.$el) // moves $el into $container
				.append(this.$front, this.$el) // moves $el into $container
		...
		handleScroll: function() {
			...
			this.$front.scrollTop(scrollTop);
			...
			this.$front.css('transform', (scrollLeft > 0) ? 'translateX(' + -scrollLeft + 'px)' : '');
		},
